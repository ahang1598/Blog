...
# Linux Centos 安装使用Java

查询可安装的版本`yum search java | grep jdk`

我这里安装的是`jdk11`无图形化服务器最小版本: `sudo yum install java-11-openjdk-headless`

安装后通过`java -version`查看安装的版本

如果需要使用`javac`编译类文件需要安装`yum install java-11-openjdk-devel`

运行某java类`javac a.java`, `java a`
...
设置连接`jconsole`来监控某个java类运行情况

```java
java -Djava.rmi.server.hostname=ip地址 -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=端口号 -Dcom.sun.management.jmxremote.ssl=是否安全连接 -Dcom.sun.management.jmxremote.authenticate=是否认证 java类
```

例如在Linux中设置，端口号自己任意定的,但是防火墙得放过该端口

```java
java -Djava.rmi.server.hostname=xx.134.13x.xx -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=40000 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false linux
```

在Windows的cmd中使用`jconsole`命令来打开图形化监控工具，java自带的，设置远程连接`xx.134.13x.xx:40000`
...
```java
    public static void main(String[] args) {
        // 精简方式二：使用lambda
        Runnable r2 = () -> { log.debug("run2");};
        Thread t2 = new Thread(r2, "t2");
        t2.start();

        // 精简方式二
        Thread t3 = new Thread( ()-> {log.debug("run3");}, "t3" );
        t3.start();
...
输出：程序在 t1 线程运行， `run()`方法里面内容的调用是**异步的** Test4.java
...
将上面代码的`thread.start();`改为 `thread.run();`输出结果如下：程序仍在 main 线程运行， `run()`方法里面内容的调用还是**同步**的
...
2. 其它线程可以使用 interrupt 方法**打断正在睡眠的线程**，那么被打断的线程这时就会抛出 `InterruptedException`异常【注意：这里打断的是正在休眠的线程，而不是其它状态的线程】
...
// t2.setPriority(Thread.MAX_PRIORITY); t1.start(); // 10
// 默认优先级为5
t2.start();
...
```mermaid
graph TD
	A[main]-->B(t1.start)-->|1s后|C(r=10)-->|t1终止|D(t1.join)
	A-->D
```
...
```java
	...
	public static void method() throws InterruptedException {
        ...
        System.out.println(end-start); // 结果为等待了2s
    }
```
...
graph TD
	A[main]-->B(t1.start)-->|1s后|C(r=10)-->|t1终止|D(t1.join)
	A-->D-->G
	A-->E(t2.start)-->|2s后|F(r=20)-->|t2终止|G(t2.join - t2仅再等待1s)
	
```



使用`join(times)`对等待的时间加以限制，超过时间就停止等待，继续运行
...
public static void test3() throws InterruptedException { Thread t1 = new Thread(() -> {
	sleep(2);
    r1 = 10;
});
...
**打断 sleep，wait，join 的线程，会清空中断状态**
...
打断正常运行的线程, **不会重置打断状态**，线程并不会暂停，只是调用方法`Thread.currentThread().isInterrupted();`的返回值为**true**，可以判断`Thread.currentThread().isInterrupted();`的值来手动停止线程
        ...
        thread = new Thread(()->{
            while(true){
                ...
                try {
                    ...
                    // 这里非常重要，当sleep时被打断会出异常，
                    // 默认会重置打断标记，这时需要我们再设置打断标志为true
...
```java
private static void test4() { Thread t1 = new Thread(() -> {
	for (int i = 0; i < 5; i++) { 
    	log.debug("park..."); 
        LockSupport.park();
		log.debug("打断状态：{}",   Thread.currentThread().isInterrupted());
...
默认情况下，java进程需要等待所有的线程结束后才会停止，但是有一种特殊的线程，叫做守护线程，在其他线程全部结束的时候**即使守护线程还未结束代码未执行完java进程也会停止**。普通线程t1可以调用`t1.setDeamon(true);` 方法变成守护线程 
...
情况3：3 1s 12 或 23 1s 1 或 32 1s 1     a和b方法公用同一把锁，c方法没有上锁
...
情况4：2 1s 后 1     a方法用的n1实例的锁，b方法用的n2实例的锁，所以两个方法没有共用一把锁
...
情况5：2 1s 后 1     a方法用的是Number类的锁，而b方法用的是类实例n1的锁，不共用一把锁
...
情况6：1s 后12， 或 2 1s后 1     a和b方法用的都是Number类的锁
...
情况7：2 1s 后 1       a方法用的Number类的锁，b方法用的n2实例的锁，两者不共用同一把锁
...
情况8：1s 后12， 或 2 1s后 1     a方法和b方法共用Number类同一把锁
...
`UserDaoImpl`类中没有成员变量，`update`方法里的变量引用的对象不是线程共享的，所以是线程安全的；

`UserServiceImpl`类中只有一个线程安全的`UserDaoImpl`类的实例，

那么`UserServiceImpl`类也是线程安全的，
